...
public:
	//Used to make the health bar have the correct health on the client side
	...
	*/
	...
	/**
	 * Getter for the CurZone
	 * @return returns the CurZone 
	 */
	AZoneTool* GetCurZone();
	
	/**
	 * Setter for the CurZone
	 * @param NewAssignedZone Set the AssignedZone to the new Zone the AI is in 
	 */
	void SetCurZone(AZoneTool* NewAssignedZone);
	...
	*/
	...
	/**
	* Weapon team implementation for ragdoll of enemies
	*/
	UFUNCTION(BlueprintCallable)
	void Ragdoll();
	...
	*/
	...
	//Reset the kill timer that removes inactive enemies
	void ResetKillTimer();

	//Starting the timer that removes inactive enemies
	void StartKillTimer();

	//Enable on enemies that do not walk around on the ground to make sure they do not destroy themeselves for no reason
	UPROPERTY(EditAnywhere, Category = "AI Settings | General Stats", meta= (AllowPrivateAccess= "true"))
	bool DisableKillTimer = false;

	
protected:
	//How many seconds an Enemy is allow to not be moving for
	float KillTimer = 10;

	//Saved off of the Kill timer so that when it gets reset it will be reset to the correct time
	float SavedKillTimer=10;

	//Bool for if the Kill timer is started or not
	...
	* Cool down for attacks
	...
	UPROPERTY(Replicated,EditAnywhere, BlueprintReadWrite, Category = "AI Settings | General Stats", meta = (AllowProtectedAccess = "true"))
	float TotalHealth;
...
private:
	...
	/**A reference to the ZoneTool the AI is currently in. Must be set for the AI to transfer to a different zone*/
	AZoneTool* CurZone;
