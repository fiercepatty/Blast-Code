		...
		if(GruntAI)
		{
			...
			if(Controller->GetPathFollowingComponent()->DidMoveReachGoal())
			{
				...
				if(GruntAI->bFireBurst)
				{
					...
					//GEngine->AddOnScreenDebugMessage(0,1.5,FColor::Red,TEXT("Trying to fire"));
				}
				else
				{
					// we have final location but we need this to be a move to call with a valid
					//GEngine->AddOnScreenDebugMessage(1,1.5,FColor::Red,TEXT("Currently Moving"));
					if (GruntAI->ShowDebug)
						DrawDebugLine(GetWorld(), GruntAI->GetActorLocation(), GruntAI->GetCurrentStrafingLocation(),
					...
					Controller->MoveToLocation(GruntAI->GetCurrentStrafingLocation(), 0);
					// set the grunt so that we are moving
					// also set the destination location
					GruntAI->SetCurrentAttackingDestination(GruntAI->GetCurrentStrafingLocation());
			...
			if(GruntAI->bFireBurst !=true && !Controller->GetPathFollowingComponent()->HasValidPath())
			{
				if (GruntAI->ShowDebug)
					DrawDebugLine(GetWorld(), GruntAI->GetActorLocation(), GruntAI->GetCurrentStrafingLocation(),
						FColor::Blue, false, 0.5f, 0, 5);
...
bool UBTTask_GruntPlayerFoundChasing::MoveToLocation(AGruntAIEnemy* GruntAI, AEnemyControllerParent* EnemyController) const
{
	if(GruntAI)
	{
		if(EnemyController)
		{
			...
			const FVector TargetPosition = VectorToPlayer * DistanceBetweenStrafes + GruntAI->GetActorLocation();
			//DrawDebugCircle(GetWorld(),TargetPosition,25,4,FColor::Blue,false,20,0,5);
