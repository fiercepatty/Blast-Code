...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AI Settings | Grunt Settings", meta = (AllowProtectedAccess = "true"))
	...
	*/
	 ...
	 */
	inline FVector GetCurrentStrafingLocation() {
		if (StrafingPathIndex<StrafingPath.Num())
			return StrafingPath[StrafingPathIndex];
		return StrafingPath[StrafingPath.Num()-1];
	}
	

protected:
	//Where the bullets are fired from
	...
	//Save off the time so that we can reset the time when needed
