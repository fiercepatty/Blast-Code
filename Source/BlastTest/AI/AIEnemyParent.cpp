...
AAIEnemyParent::AAIEnemyParent()
{
	...
	//Used to make sure that the nav volume is only around the ai and not anywhere else
	if(!InvokerComponent)
	{
...
void AAIEnemyParent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	//Replicate current health.
	DOREPLIFETIME(AAIEnemyParent, CurrentHealth);
	DOREPLIFETIME(AAIEnemyParent, TotalHealth);
...
void AAIEnemyParent::Tick(float DeltaTime)
{
	...
	if(!DisableKillTimer)
	{
		...
		if (Movement)
		{
			//Start the kill timer if the enemy is not moving on the ground and we give a buffer on the time because the AI could be sitting still between patrol points or actively shooting
...
AZoneTool* AAIEnemyParent::GetCurZone()
{
	return CurZone;
}


void AAIEnemyParent::SetCurZone(AZoneTool* NewAssignedZone)
{
	CurZone = NewAssignedZone;
}
...
void AAIEnemyParent::ResetKillTimer()
{
	bKillTimer=false;
	KillTimer=SavedKillTimer;
}

void AAIEnemyParent::StartKillTimer()
{
	bKillTimer=true;
}
