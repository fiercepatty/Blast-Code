...
void AFlamethrowerProjectile::Tick(float DeltaSeconds)
{
	...
	if(this->ScaleInterval <= 0.0)
	{
		...
		*/
		...
		//Scale down the damage as it goes out.
		DamageAmount = static_cast<int>(DamageAmount * DamageScaleDownPercent);
...
void AFlamethrowerProjectile::BeginPlay()
{
	Super::BeginPlay();
}
                                        ...
                                        UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if(OtherActor->Tags.Contains("Enemy"))
	{
		...
		if (Enemy)
		{
			...
			//if (GEngine)
				//GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Yellow, FVector(DamageAmount).ToString());
